...
for (day in unique(flight_id$FP_Date) %>% .[start_day_num:length(.)]) {
  ...
  for (i in 1:length(day_flight_id)) {
    ...
    wake_i <- if (!(lookup_ac_type %in% c("A339", "B78X", "E295", "SF50"))) {
      wake_data[aircraft_type == unique(tracks$Follower_Aircraft_Type)][1]
    } else if (lookup_ac_type %in% c("A339", "B78X")) {
      data.table(aircraft_type = lookup_ac_type, wake = "B")
    } else if (lookup_ac_type %in% c("E295")) {
      data.table(aircraft_type = lookup_ac_type, wake = "E")
    } else if (lookup_ac_type %in% c("SF50")) {
      data.table(aircraft_type = lookup_ac_type, wake = "F")
    }
